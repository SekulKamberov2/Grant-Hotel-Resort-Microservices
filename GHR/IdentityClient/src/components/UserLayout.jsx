    ...
    return (
        <LayoutContainer>
            <Navbar style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <NavbarLeft style={{ display: 'flex', alignItems: 'center', fontWeight: 500 }}>
                ...
                <NavbarRight>
                    {user.roles?.length > 0 && (
                        <>
                            {ADMIN && (
                                <RoundedButton width="80px" onClick={() => navigate('/signup')}>
                                    SignUp
                                </RoundedButton>
                            )}
                            {(ADMIN || MANAGER) && (
                    ...
                    <RoundedButton as="button" onClick={() => navigate('/profile')}>
                        Profile
                    </RoundedButton>
                    <RoundedButton as="button" onClick={handleLogout}>
                        Logout
                    </RoundedButton>
                </NavbarRight>
            </Navbar>
